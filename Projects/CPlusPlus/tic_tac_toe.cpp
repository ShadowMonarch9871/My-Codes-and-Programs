...
public:
  ...
  int getFilledCellCount() const { return filledCells; }
};
class TicTacToe {
private:
  Board board;
    ...
    while (!board.isFull()) {
      ...
      while (true) {
        cout << currentPlayer.getName() << " (" << currentPlayer.getSymbol()
             << "), enter row number(1-3) and col number(1-3)";
        cin >> row >> col;
        row--;
        col--;
        if (board.isValidMove(row, col)) {
          ...
          cout << "Enter Valid row and col";
        }
      }
      board.makeMove(row, col, currentPlayer.getSymbol());
      if (board.checkWin(currentPlayer.getSymbol())) {
        board.drawBoard();
        cout << currentPlayer.getName() << " wins the match!!!!!";
        return;
      }
      switchTurn();
    }
